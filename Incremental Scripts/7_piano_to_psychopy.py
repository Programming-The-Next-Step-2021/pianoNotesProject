...
# Now I will create two dataframes: The first one 'key_bindings' has the note files and the corresponding key_id that
# we implemented earlier The second "condition_df" dataframe will include the order in which the notes will be shown
# (random for now) and store the reaction time

key_bindings = read_csv("./piano_config_67k.csv", index_col=0)

# condition_df = key_bindings.sample(frac=1).head(10)
condition_df = key_bindings.head(10)
midi.init()
...
for idx, row in condition_df.iterrows():
    ...
    stim_img = ImageStim(win, "./Notes_piano/" + current_note)
